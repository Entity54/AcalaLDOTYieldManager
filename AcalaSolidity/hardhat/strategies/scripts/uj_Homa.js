...
async function main() {
  ...
  console.log("Inital %s ACA balance: %s ACA", deployer.address, initialAcaBalance.toString());
  console.log("Inital %s AUSD balance: %s AUSD", deployer.address, initialAusdBalance.toString());
  // const amountAUSDtoswapToACA = parseUnits("1", 12)
  const amountAUSDtoswapToACA = parseUnits("1", 1)

  // console.log(`Swapping ${amountAUSDtoswapToACA} to ACA`);
  // await DEXinstance.swapWithExactSupply([LDOT, AUSD], amountAUSDtoswapToACA , 1)
  console.log(`************************** SWAP ******************************`);
  ...
  const one1 = ethers.BigNumber.from(`${ parseUnits("1", 20)}`);
  const precision = ethers.BigNumber.from(`${ parseUnits("1", 8)}`);
  const presdot =  ethers.BigNumber.from(`${ parseUnits("1", 10)}`);
  let mandala_DOT_LDOT_WEI1 = ((one1.mul(precision)).div(homaExchangeRate));
  let dotintermsofldotWEI = (initialDotBalance.mul(mandala_DOT_LDOT_WEI1)).div(presdot);
  let dotintermsofldot = formatUnits(dotintermsofldotWEI, 10);
  console.log(`********************************************************`);
  console.log(`===. one1: ${one1.toString()} precision:${precision} homaExchangeRate: ${homaExchangeRate.toString()} mandala_DOT_LDOT_WEI1: ${mandala_DOT_LDOT_WEI1.toString()} initialDotBalance: ${initialDotBalance} dotintermsofldotWEI: ${dotintermsofldotWEI} dotintermsofldot: ${dotintermsofldot}`);
  console.log(`********************************************************`);
  console.log(`********************************************************`);
  console.log(`********************************************************`);
  // ===. one1: 100000000000000000000 precision:100000000 homaExchangeRate: 110574281655845687 mandala_DOT_LDOT_WEI1: 90436942933 initialDotBalance: 9356631690868 dotintermsofldotWEI: 84618516627212 dotintermsofldot: 8461.8516627212
